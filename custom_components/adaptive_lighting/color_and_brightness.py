...
class SunEvents:
    ...
    def sun_position(self, dt: datetime.datetime) -> float:
        ...
        # PERIOD=90
        # return math.cos(2*math.pi*dt/PERIOD)
...
class SunLightSettings:
    ...
    def _calc_accented_color_interpolation(
        self, daylight_color_rgb: tuple[float, float, float], accent_color_rgb: tuple[float, float, float], accent_percent: float
    ) -> tuple[float, float, float]:
        if accent_color_rgb is None:
            return None
        
        # Linear interpolation in XY-B color space, makes more sense
        color_upper_temp = self.color_temp_kelvin(self.accent_color_upper_bound)
        color_upper_rgb = color_temperature_to_rgb(color_upper_temp)
        bezier_point_xyb = color_RGB_to_xy_brightness(*color_upper_rgb)
        accent_color_xyb = color_RGB_to_xy_brightness(*accent_color_rgb)
        control_point1 = color_RGB_to_xy_brightness(*daylight_color_rgb)
    ...
    def _calc_accented_color(
        self, percent: float, daylight_color_rgb: tuple[float, float, float],
              accent_color_rgb: tuple[float, float, float]
    ) -> tuple[float, float, float]:
        if accent_color_rgb is None:
            return daylight_color_rgb
    ...
    def brightness_and_color(
        ...
        is_sleep: bool,
        use_accent: bool,
        accent_color_rgb: tuple[float, float, float] | None,
    ) -> dict[str, Any]:
        ...
        if use_accent and (accent_color_rgb is not None) and (not is_sleep):
            rgb_color = self._calc_accented_color(sun_position, rgb_color, accent_color_rgb)
        ...
        return {
            ...
            ATTR_SWITCH_ACCENT_COLOR: accent_color_rgb,
        }
    ...
    def get_settings(
        ...
        return self.brightness_and_color(dt, is_sleep, use_accent, accent_color_rgb)
